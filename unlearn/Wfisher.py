...
def Wfisher(data_loaders, model, criterion, args, mask=None, **kwargs):
    start_time = time.time() #Initialize unlearn time for Wfisher
    ...
    end_time = time.time()
    unlearn_time_ms = (end_time - start_time) * 1000  # Calculate unlearning time in milliseconds
    print(f"Unlearning time: {unlearn_time_ms} ms")
    callback = kwargs.get('callback', None)
    if callback is not None:
        epoch = 'final'  # Wfisher doesn't iterate over epochs so we use 'final'
        callback(data_loaders, epoch, model, args, criterion, unlearn_time_ms,**kwargs)
